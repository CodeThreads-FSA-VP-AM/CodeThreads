...
const Products: React.FC<Props> = ({ setProductId, user }) => {
  ...
  // find out how to get the index of the products in the currentpage
  // how to get the index of the previous page  x
  ...
  // filter the products to only show the set products perpage
  const totalProducts = filteredProducts.length;
  const totalPages = Math.ceil(totalProducts / productsPerPage);

  // page handler to changing the page of products
            ...
            <nav className='flex items-center justify-between border-t border-gray-200 bg-white px-4 py-3 sm:px-6' aria-label='Pagination'>
              <div className='hidden sm:block'>
                <p className='text-sm text-gray-700'>
                  Showing <span className='font-medium'>1</span> to <span className='font-medium'>10</span> of <span className='font-medium'>20</span>{' '}
                  results
                </p>
              </div>
