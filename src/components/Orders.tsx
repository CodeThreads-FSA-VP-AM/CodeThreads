...
import { fetchOrder, deleteOrder, fetchUser, createOrder, checkoutOrder, fetchProductById } from '../api/api';
...
const Orders: React.FC<Props> = ({ setProductsLength }) => {
  ...
  const handleDeleteCartItem = async (productId: number) => {
    try {
      ...
      setCart(updatedCart);
      const updatedProducts = await Promise.all(updatedCart.map((item) => fetchProductById(item.id)));
  ...
  useEffect(() => {
    if (cart.length > 0) {
      ...
      const fetchProducts = async () => {
        try {
          const fetchedProducts = await Promise.all(productIds.map((id) => fetchProductById(id)));
  ...
  return (
    <>
      {loading ? (
        ...
        <div>
          <div className='fixed top-0 w-full h-full overflow-x-hidden overflow-y-auto bg-black bg-opacity-90 sticky-0' id='chec-div'>
            <div
              ...
              <div className='flex flex-col justify-end md:flex-row' id='cart'>
                ...
                >
                  ...
                  {orders
                    ...
                    .map((o: Order, idx) => (
                      <div className='items-center py-8 border-t border-gray-200 md:flex mt-14' key={idx}>
                        <div className='w-1/4'>
                          <img src={o.front_url} alt='...' className='object-cover object-center w-full h-full' />
                        </div>
                        <div className='md:pl-3 md:w-3/4'>
                          <p className='pt-4 text-xs leading-3 text-gray-800 md:pt-0'>{1 + idx++}</p>
                          <div className='flex items-center justify-between w-full pt-1'>
                            <p className='text-base font-black leading-none text-gray-800 capitalize'>{o.title}</p>
                            <label className='p-1 border border-gray-200 focus:outline-none'>
                              <input type='number' min='1' max='10' defaultValue={o.quantity} className='border-2 border-gray-500 '></input>
                            </label>
                          </div>

                          <p className='pt-2 text-xs leading-3 text-gray-600 capitalize'>{o.description}</p>
                          <p className='py-4 text-xs leading-3 text-gray-600'>Color: Black</p>
                          <p className='text-xs leading-3 text-gray-600 capitalize w-96'>{o.status}</p>
                          <div className='flex items-center justify-between pt-5 pr-6'>
                            ...
                            <p className='text-base font-black leading-none text-gray-800'>${o.price * o.quantity}</p>
                          </div>
                        </div>
                      </div>
                    ))}
                  {product?.map((p: Product, idx) => {
                    const cartItem = cart.find((item: CartItem) => item.id === p.id);
                    const quantity = cartItem ? cartItem.quantity : 0;

                    return (
                      <div className='items-center py-8 border-t border-gray-200 md:flex mt-14' key={idx}>
                        <div className='w-1/4'>
                          <img src={p.front_url} alt='...' className='object-cover object-center w-full h-full' />
                        </div>
                        <div className='md:pl-3 md:w-3/4'>
                          <p className='pt-4 text-xs leading-3 text-gray-800 md:pt-0'>{1 + idx}</p>
                          <div className='flex items-center justify-between w-full pt-1'>
                            <p className='text-base font-black leading-none text-gray-800 capitalize'>{p.title}</p>
                            <label className='p-1 border border-gray-200 focus:outline-none'>
                              <input type='number' min='1' max='10' defaultValue={quantity} className='border-2 border-gray-500 '></input>
                            </label>
                          </div>

                          <p className='pt-2 text-xs leading-3 text-gray-600 capitalize'>{p.description}</p>
                          <p className='py-4 text-xs leading-3 text-gray-600'>Color: Black</p>
                          <p className='text-xs leading-3 text-gray-600 capitalize w-96'>{p.status}</p>
                          <div className='flex items-center justify-between pt-5 pr-6'>
                            <div className='flex itemms-center'>
                              <p className='text-xs leading-3 text-gray-800 underline cursor-pointer'>Add to favorites</p>
                              <button
                            ...
                            <p className='text-base font-black leading-none text-gray-800'>${(p.price * quantity).toFixed(2)}</p>
                          </div>
                        </div>
                      </div>
                    );
                  ...
                  <div className='w-full h-full bg-gray-100 xl:w-1/2 md:w-1/3'>
                    <div className='flex flex-col justify-between py-20 overflow-y-auto md:h-screen px-14'>
                      ...
                      <div>
                        ...
                        <NavLink to={{ pathname: '/checkout' }} state={{ totalPrice }}>
                          <button
